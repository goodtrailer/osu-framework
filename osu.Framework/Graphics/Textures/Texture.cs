...
namespace osu.Framework.Graphics.Textures
{
    public class Texture : IDisposable
    {
        ...
        internal virtual void SetData(ITextureUpload upload, WrapMode wrapModeS, WrapMode wrapModeT, Opacity? opacity)
        {
            ...
            if (upload.Bounds.IsEmpty && upload.PixelCount > 0)
            {
                upload.Bounds = new RectangleI(0, 0, Width, Height);

                if (upload.Bounds.Width * upload.Bounds.Height > upload.PixelCount)
                {
                    throw new InvalidOperationException(
                        $"Size of texture upload ({upload.Bounds.Width}x{upload.Bounds.Height}) does not contain enough data ({upload.PixelCount} < {upload.Bounds.Width * upload.Bounds.Height})");
                }
            }
